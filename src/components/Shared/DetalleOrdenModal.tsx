...
export default function DetalleOrdenModal({ orden, onClose }: DetalleOrdenModalProps) {
  ...
  return (
    <Modal titulo={`Pedido · ${etiquetaMesa(orden.mesa_numero)}`} onClose={onClose}>
      <div className="mb-4 grid grid-cols-2 gap-3 rounded-lg bg-slate-50 px-3 py-3">
          ...
          <tbody>
            {items.map((it) => (
              <tr key={it.id} className="border-t border-slate-100">
                <td className="px-3 py-2 text-slate-700">{it.item_nombre}</td>
                <td className="px-3 py-2 text-center text-slate-500">{it.cantidad}</td>
                <td className="px-3 py-2 text-right text-slate-500">{soles(it.item_precio)}</td>
